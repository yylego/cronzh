...
func TestNewS定时任务列表_6字段秒级精度(t *testing.T) {
	...
	s定时任务列表 := crontaskzh.NewS定时任务列表([]*crontaskzh.T定时任务{
		{
			E任务名称:    "XXX",
	...
	require.NoError(t, s定时任务列表.Debug(cronnextzh.P带秒数的表达式解析器, 10))
}
...
func TestNewS定时任务列表_5字段分钟精度(t *testing.T) {
	...
	require.NoError(t, s定时任务列表.Debug(cronnextzh.P只到分的表达式解析器, 10))
}

func TestS定时任务列表_Debug(t *testing.T) {
	v定时任务 := &crontaskzh.T定时任务{
		S定时表达式列表: []string{
	...
	s定时任务列表 := crontaskzh.NewS定时任务列表([]*crontaskzh.T定时任务{v定时任务})
	require.NoError(t, s定时任务列表.Debug(cronnextzh.P带秒数的表达式解析器, 10))
}
...
func TestS定时任务列表_Set注册定时任务(t *testing.T) {
	...
	require.NotEmpty(t, s定时任务列表)

	cron := cronv3.New(cronv3.WithSeconds(), cronv3.WithLocation(time.UTC))
	require.NoError(t, s定时任务列表.Set注册定时任务(cron))
	cron.Start()
...
func TestS定时任务列表_Set注册定时任务_错误表达式(t *testing.T) {
	s定时任务列表 := crontaskzh.NewS定时任务列表([]*crontaskzh.T定时任务{
		{
			E任务名称:    "坏的",
			S定时表达式列表: []string{"bad expression"},
			F执行函数:    func(e事件名称 string) {},
		},
	})
	cron := cronv3.New(cronv3.WithSeconds())
	err := s定时任务列表.Set注册定时任务(cron)
	require.Error(t, err)
	t.Log("expected:", err)
}
