...
func (cs S定时任务列表) Set注册定时任务(cron *cron.Cron) error {
	...
	for idx, one := range cs {
		...
		for _, spec := range one.S定时表达式列表 {
			...
			if _, err := cron.AddFunc(spec, func() {
				...
				return erero.WithMessagef(err, "注册定时任务失败: spec=%s name=%s", spec, e任务名称)
			}
		}
	}
...
// nDate: 要显示的天数
func (cs S定时任务列表) Debug(p表达式解析器 *cronnextzh.P表达式解析器, nDate int) error {
	type T任务和时间 struct {
		E任务名称 string
		T执行时间 time.Time
	}
	...
	for _, sa := range cs {
		for _, spec := range sa.S定时表达式列表 {
			times, err := p表达式解析器.Get获取未来N天内的执行时间(spec, now, nDate)
			if err != nil {
				return erero.WithMessage(err, "获取执行时间失败")
			}
			for _, v执行时间 := range times {
